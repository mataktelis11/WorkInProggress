...
//reads from the user input an int / keeps asking until the input is int in [1,5]
int getInt(int i)
{
	string s;
	while(true)
...
//function that finds the height of a node for delmax(case 2)
int depth(Node* node)
{
	if (node == NULL)
		return 0;
	else
	{
		...
		int lDepth = getHeight(node->left);		//we will use this function in case 2 of delmax as we do not change the height of any left subtree
		int rDepth = depth(node->right);

		//height is max of the children heights
		if (lDepth > rDepth)
			return(lDepth + 1);
...
Node* delMax(Node* root)
{
	...
	else
	{
		...
		//find the new max
		findNewMax(root);
		//insert the nodes of the removed subtrees
