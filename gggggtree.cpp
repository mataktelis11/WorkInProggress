...
//reads from the user input an int / keeps asking until the input is int is in [1,4]
int getInt(int i)
{
...
Node* maxNode = NULL;
void getMax(){cout<<"Max node is: "<<maxNode->key<<endl;}
void findNewMax(Node* root)
{
	...
	{
		root = root->right;
		if(root->key > maxval)maxval=root->key;
	}
	//cout<<maxval<<endl;
	maxNode = root;
}
